...
void RadiationFieldProbe::probe()
{
    ...
    if (config->hasRadiationField())
    {
        ...
        else
        {
            ...
            bridge.writeQuantity("J", units->umeanintensity(), "mean intensity", "density-weighted mean intensity",
                                 axis, units->uwavelength(), addColumnDefinitions, valueInCell, weightInCell);
