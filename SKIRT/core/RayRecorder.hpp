...
#ifndef RAYRECORDER_HPP
#define RAYRECORDER_HPP
...
/** RayRecorder is a helper class used by instruments to actually record the effects of detected
    photon packets. Each Instrument instance employs a single RayRecorder instance.

    Features
    --------

    Depending on an instrument's needs, a RayRecorder instance records an %SED (spectral energy
    ...
    A RayRecorder instance expects a rigourous calling sequence. During setup, the instrument
    configures the RayRecorder's operation, specifying the wavelength grid, the items to be
    ...
    A RayRecorder instance handles the conversion of the detected photon packet contributions to a
    ...
    from the instrument, with a wide dynamic range. The RayRecorder class does not support
    ...
    A RayRecorder instance dynamically adjusts its memory allocation to the configuration and
...
class RayRecorder final
{
    //============= Construction - Setup - Destruction =============

public:
    /** The constructor initializes the RayRecorder to a configuration that records nothing. The
    ...
    RayRecorder(const SimulationItem* parentItem);
...
public:
    ...
    void trace(PhotonPacket* pp, int l);
